...
def get_tweets_from_couchdb(geohash):
  counter = 0
  start_geohash = geohash + '0'
  end_geohash = geohash + 'z'
  while True:
    url = 'http://readonly:ween7ighai9gahR6@45.113.232.90/couchdbro/twitter/_design/twitter/_view/geoindex?include_docs=true&reduce=false&start_key=["' + start_geohash + '",2014,1,1]&end_key=["' + end_geohash + '",2017,12,31]&skip='+ str(100*counter) +'&limit=100'
    print(url)
    r = requests.get(url)
    json_data = r.json()
    rows = json_data['rows']
...
if __name__ == '__main__':
  ...
  for geohash in geohash_list:
    get_tweets_from_couchdb(geohash)
